...
class AffinityTrackingService
{
    ...
    public function trackInteraction(string $cookieHash, int $storyId, string $interactionType): void
    {
        try {
            ...
            foreach ($story->tags as $tag) {
                $this->updateTagAffinity($cookieHash, $tag->id, $interactionType);
            }
        } catch (\Exception $e) {
    ...
    private function updateTagAffinity(string $cookieHash, int $tagId, string $interactionType): void
    {
        $weight = self::INTERACTION_WEIGHTS[$interactionType] ?? 1.0;

        DB::table('user_tag_affinities')
            ->updateOrInsert(
                ['cookie_hash' => $cookieHash, 'tag_id' => $tagId],
                [
