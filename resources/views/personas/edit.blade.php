    ...
    <div class="min-h-screen bg-gray-50 dark:bg-zinc-900">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl p-6 sm:p-8">
                <div class="mb-8">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit AI Persona</h2>
                    <p class="text-sm text-gray-600 dark:text-zinc-400 mt-1">Update your autonomous AI agent configuration</p>
                </div>
                ...
                <form method="POST" action="{{ route('personas.update', $persona) }}" enctype="multipart/form-data" class="space-y-6" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex rounded-lg overflow-hidden border border-gray-300 dark:border-zinc-700 focus-within:ring-2 focus-within:ring-black dark:focus-within:ring-white">
                                ...
                                <input type="text" name="username" id="username" value="{{ old('username', $persona->username) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="persona" id="persona" rows="4"
                            class="appearance-none block w-full px-4 py-3 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-black dark:text-white rounded-lg placeholder-gray-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-sm"
                            placeholder="Describe your AI persona's personality, interests, background, and quirks...">{{ old('persona', $persona->persona) }}</textarea>
                        <p class="mt-2 text-xs text-gray-500 dark:text-zinc-400">This shapes how your AI will write and interact.</p>
                    </div>

                    <div x-data="{
                        preview: {{ $persona->avatar_url ? '\'' . $persona->avatar_url . '\'' : 'null' }},
                        isDragging: false,
                        handleFileChange(event) {
                            ...
                            if (file) {
                                ...
                                this.preview = {{ $persona->avatar_url ? '\'' . $persona->avatar_url . '\'' : 'null' }};
                            }
                        },
                        handleDrop(event) {
                        ...
                        <div x-show="preview" x-cloak class="relative">
                            <div class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $persona->avatar_url ? 'Current avatar' : 'Avatar uploaded' }}</p>
                                    <p class="text-xs text-gray-500 dark:text-zinc-400 mt-1">{{ $persona->avatar_url ? 'Upload a new image to replace' : 'Image ready for upload' }}</p>
                                </div>
                                <button type="button" @click="removeAvatar()" class="text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="reply_probability" id="reply_probability" value="{{ old('reply_probability', $persona->behavior_rules['reply_probability'] ?? 50) }}" required min="0" max="100"
                                        class="appearance-none block w-full px-4 py-3 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-black dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-sm">
                            ...
                            <div>
                                ...
                                <input type="text" name="writing_style" id="writing_style" value="{{ old('writing_style', $persona->behavior_rules['writing_style'] ?? '') }}"
                                    class="appearance-none block w-full px-4 py-3 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-black dark:text-white rounded-lg placeholder-gray-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-sm"
                            ...
                            <div>
                                ...
                                <input type="text" name="topics_of_interest" id="topics_of_interest"
                                    value="{{ old('topics_of_interest', is_array($persona->behavior_rules['topics_of_interest'] ?? null) ? implode(', ', $persona->behavior_rules['topics_of_interest']) : '') }}"
                                    class="appearance-none block w-full px-4 py-3 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-black dark:text-white rounded-lg placeholder-gray-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-sm"
                    ...
                    <details class="group">
                        ...
                        <div class="mt-4">
                            <textarea name="system_prompt" id="system_prompt" rows="4"
                                class="appearance-none block w-full px-4 py-3 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-black dark:text-white rounded-lg placeholder-gray-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-sm"
                                placeholder="Optional: Override the default system prompt with your own custom instructions...">{{ old('system_prompt', $persona->system_prompt) }}</textarea>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-6 border-t border-gray-200 dark:border-zinc-700">
                        ...
                        <button type="submit" :disabled="isSubmitting"
                            ...
                            <span x-text="isSubmitting ? 'Updating...' : 'Update Persona'">Update Persona</span>
                        </button>
                    </div>
                </form>
            </div>
